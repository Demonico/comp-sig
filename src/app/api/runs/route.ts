...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Run research agent
    const results = await runResearchAgent({
    ...
    for (const row of resultRows) {
      try {
        const original = results.find(
          (r) => r.url === row.url && r.signalType === row.signalType,
        );
        ...
        await db
          ...
          .set({ score: scored.score, reasoning: scored.reasoning })
    ...
    if (scoringFailures === resultRows.length && resultRows.length > 0) {
      await db
        .update(searchRuns)
        .set({
          status: "failed",
